...
class P:
    ...
    WHEAT_RESERVE_CAP = 62        # wheat hogging the shed is what destroys produce
    WHEAT_CARRY = 12

    LAND_RESERVE = 700
    LAND_LAST_DAY = 23

    ANIMAL_MIN_PROFIT = 260       # marginal profit needed to buy one more
    CROP_MIN_SCORE = 4.0
...
class Brain:
    ...
    def _animal_orders(self, cash):
        ...
        for _ in range(min(6, headroom)):
            best, best_profit = None, P.ANIMAL_MIN_PROFIT
            for animal in ANIMALS:
                if ANIMALS[animal]["cost"] > budget:
                    continue
                already = (self.have[animal] + self.pending_animals.get(animal, 0)
                           + counts[animal])
                profit = self._animal_marginal(animal, counts[animal])
                # Never buy an animal we have nowhere to put.
                if profit > best_profit and self._structure_room(animal, counts):
                    best, best_profit = animal, profit
            if best is None:
                break
            counts[best] += 1
    ...
    def _crop_score(self, crop):
        """Coins per (tile-day + action) for starting one cycle of `crop` now."""
        cd = CROPS[crop]
        use_fert = (self.shed.get("FERTILIZER", 0) + self.carried.get("FERTILIZER", 0) > 2
                    and self.proj_price("FERTILIZER") < 70)
        units, days, acts = CROP_CYCLE[crop]["fert" if use_fert else "plain"]
        ...
        if use_fert:
            profit -= self.proj_price("FERTILIZER")
        return profit / float(days + acts)

    def _crop_ranking(self):
        ranked = []
        for crop in CROPS:
            if crop == "MELON":
                if (self.melon_units_planted >= 160
                        or self.proj_price("MELON", 3.0) < 60):
                    continue
            s = self._crop_score(crop)
            if s > P.CROP_MIN_SCORE:
                ranked.append((s, crop))
    ...
    def _tile_plan(self):
        ...
        ranked = self._crop_ranking()
        if not ranked:
            return plan
        # Spread the best crop over the remaining land, but cap melon by market.
        melon_room = max(0, (160 - self.melon_units_planted) // CROPS["MELON"]["max_yield"])
        counts = {}
        while idx < len(free):
            placed = False
            for _, crop in ranked:
                if crop == "MELON" and counts.get("MELON", 0) >= melon_room:
                    continue
    ...
    def _seed_orders(self, cash):
        """Hold a few days of seed for the crops we actually want."""
        orders = []
        ranked = self._crop_ranking()
        if not ranked:
            return orders
        free = len(self.empty_tiles)
        # Look ahead: tiles about to be freed by harvests count too.
        soon = sum(1 for _, _, t in self.plants
                   if self.day - t["planted_day"] >= CROPS[t["crop"]]["max_yield_day"])
        want_total = min(free + soon + 3, 26)
        budget = cash
        for _, crop in ranked:
            if want_total <= 0 or budget < CROPS[crop]["seed"]:
                break
            if crop == "MELON":
                cap = max(0, (160 - self.melon_units_planted) // CROPS["MELON"]["max_yield"])
                if cap <= 0:
                    continue
                want = min(want_total, cap)
            else:
                want = want_total
            have = self.seeds.get(crop, 0)
            n = int(min(max(0, want - have), budget // CROPS[crop]["seed"]))
            if n > 0:
                orders.append(["BUY_SEED", crop, n])
                budget -= n * CROPS[crop]["seed"]
                want_total -= n
            # Leave room for the runner-up crops rather than hoarding one seed type.
            want_total = min(want_total, 10)
        return orders
    ...
    def _wheat_reserve(self):
        if self.days_left < 0:
            return 0
        need = self.n_animals * P.WHEAT_RESERVE_DAYS
        return int(min(need, P.WHEAT_RESERVE_CAP, self.n_animals * (self.days_left + 1)))
    ...
    def _target_hands(self):
        ...
        work += 2.0 * min(len(self.empty_tiles), 12) + 1.5 * len(self.weeds)
        work *= 1.5                                        # walking and shed trips
    ...
    def _plan_buys(self):
        ...
        need = self._wheat_reserve()
        have = self.shed.get("WHEAT", 0) + self.carried.get("WHEAT", 0)
        deficit = need - have
        if deficit > 0 and self.days_left >= 0:
            price = self._wheat_unit_cost()
            limit = P.WHEAT_BUY_MAX_PRICE
            if self.n_animals and have < self.n_animals * 0.75:
        ...
        if (extra < len(LAND_PRICES) and self.day <= P.LAND_LAST_DAY
                and self.days_left >= 5 and len(self.empty_tiles) <= 6):
    ...
    def _tasks(self):
        ...
        for x, y, t in self.plants:
            ...
            if cd["ongoing"]:
                ...
                produces_today = (age >= cd["first_yield_day"]
            ...
            if (self.days_left >= 2 and not endgame
                ...
                extra, ok = 0, False
                if cd["ongoing"]:
                    nxt = cd["first_yield_day"] + cd["interval"] * max(
                        0, -(-(age - cd["first_yield_day"]) // cd["interval"]))
                    if age >= cd["first_yield_day"] - 1 and nxt - age <= 2:
                        extra, ok = 2, True         # covers two scheduled ticks
                else:
                    ws = (cd["max_yield_day"] + 1) // 2
    ...
    def _shed_errand(self, u):
        ...
        # Everything past the shed cap is destroyed at end of day, and SELL can
        # only reach the shed -- so getting produce home is real money.
        ...
        if unfed and inv.get("WHEAT", 0) == 0 and self.shed.get("WHEAT", 0) > 0 and not self.endgame:
            urgent = any(t.get("consecutive_unfed", 0) >= 1 for t in unfed)
            value = max(value, 1400.0 if urgent else 200.0)
